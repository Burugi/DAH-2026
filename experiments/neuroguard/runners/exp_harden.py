# -*- coding: utf-8 -*-
"""특이점 넘기(방어측): 신종 발굴(exp_singularity)이 찾은 최강 공격 = 재밍포화+소량웜.
그 신종을 코디네이터 vs 통합방어(per-segment+de-jam+relay)로 붙여, 통합방어가 신종의
gap을 닫는지(=방어가 특이점을 따라잡는지) 측정. exp_ultimate 롤아웃 재사용."""
import sys, os
...
# 발굴된 신종 = 재밍 포화(~90%) + 소량 웜(~10%)
THREATS = [("재밍(기존 단일)", ["J"], 0),
           ("★신종 재밍포화+웜", ["J"] * 9 + ["W"], 0),
           ("동시(W+J+B)", ["W", "J", "B"], 0)]
print("=== 특이점 넘기: 신종 vs 코디네이터 vs 통합방어 (곱셈종합) ===", flush=True)
print("위협".ljust(20) + "코디네이터   통합방어   판정")
rows = []
for name, vec, K in THREATS:
    ...
    verd = "↑닫음" if ul > co + 0.01 else ("=" if abs(ul - co) <= 0.01 else "↓악화")
    rows.append((name, round(co, 3), round(ul, 3)))
    print(f"{name:20}{co:.3f}       {ul:.3f}     {verd}", flush=True)
with open(os.path.join(OUT, "summary_harden.csv"), "w", newline="", encoding="utf-8") as f:
    wr = csv.writer(f); wr.writerow(["threat", "coordinator", "ultimate"]); wr.writerows(rows)
print("DONE_HARDEN")
