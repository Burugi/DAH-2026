...
실험 도구임. 중심 방어 모델은 **HVT(Hypothesis-Verify-Trigger, `src/agents/jy_hvt.py`)**이고,
`rule`·`llm`·`rl` 기반 에이전트의 3×3 비교를 베이스라인으로 제공함. RAG 관련 실험(공격 유형 판단,
방어 권고)은 별도 env를 쓰므로 `appendix/`에 보관함.
...
lab/
├─ src/
│  ├─ score.py                        방어 모델(DefensePolicy) 채점 러너
│  ├─ run.py sweep.py analyze.py gallery.py make_dataset.py
│  ├─ agents/
│  │   ├─ jy_hvt.py                   HVT — 메인 방어 모델
...
├─ appendix/                          RAG 실험 (별도 env)
│  ├─ attack_rag/  defense_rag/       RAG-A(공격유형) · RAG-B(방어권고)
│  └─ rag_guided.py  rag_playbook.py  RAG 연계 방어 · RAG-lite 플레이북
