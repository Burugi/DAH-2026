...
- `src/scenarios/A01~A21.yaml` — 시나리오별 상세 정의(MITRE 태그, `attacks` 물리 파라미터). `run.py --scenario`가 로더로 병합한다. 이 문서가 설명하는 형식.
- `src/configs/attack_scenarios.yaml` — 방어 모델(HVT·reach2)용 압축 정의(vectors·frag_K·detector_q 등). `score.py --scenario`가 사용한다.
...
| `worm` | `run.py` `_worm_step()` | 웜 전파 엔진 파라미터(dormancy·spread_interval·max_infected) |
...
```bash
# 시나리오 주입 단일 실행
python src/run.py src/configs/sweep.yaml --red rule --blue rl --scenario A7

# HVT·reach2 방어 모델은 configs/attack_scenarios.yaml을 사용
python src/score.py --model hvt --scenario A7
```
