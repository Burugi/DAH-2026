...
`src/agents/jy_hvt.py` — Hypothesis-Verify-Trigger. belief 상위를 무조건 재장악하지 않고, 각 후보의
...
| 파일 | 클래스 | 방식 | 방어점수 |
|---|---|---|:--:|
| `agents/jy_hvt.py` | HVTDefense | belief 예측 + 반사실 검증(Δ>τ) 통과분만 재장악 → 오탐 회피 | ~0.92 (메인) |
...
configs/attack_scenarios.yaml                    configs/sweep.yaml
        │                                              │
        ▼                                              ▼
   score.py  ──spec──►  defense_base.DefensePolicy   sweep.py ── 9 매치업 (red×blue)
        │                  (HVTDefense · ReachV2)       │
...
`score.py`는 방어점수(+옵션 step CSV)를 출력한다. 대시보드/애니메이션 산출물은 `run.py`/`sweep.py`가
만드는 `results/<run>/` 번들에서 나오며, `docs/sample_run/`은 그렇게 만든 HVT A1 예시다.
...
- **`agents/jy_hvt.py` · `agents/reach2.py` · `agents/defense_base.py`** — HVT/단순 재장악 방어와 공용 베이스.
...
- **`score.py`** — DefensePolicy 방어 모델 채점 러너.
...
`appendix/`는 RAG 실험을 보관한다 — `attack_rag/`(RAG-A, 공격유형 판단), `defense_rag/`(RAG-B, 방어권고),
`rag_guided.py`(RAG 연계 방어), `rag_playbook.py`(시그니처 최근접 플레이북 검색 RAG-lite). 별도 env가
필요해 제출 실행 경로에는 포함되지 않는다.
